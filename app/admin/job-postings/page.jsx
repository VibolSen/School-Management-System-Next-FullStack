...
import JobPostingForm from "@/components/admin/JobPostingForm"; // Changed import path
...
export default function JobPostingsPage() {
  ...
  const fetchJobPostings = async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/admin/job-postings"); // Changed API endpoint
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/admin/job-postings/${jobToDelete}`, { // Changed API endpoint
        method: "DELETE",
      });
      const data = await res.json();
      if (!res.ok) {
  ...
  return (
    <div className="container mx-auto p-6">
      <h1 className="text-3xl font-bold text-gray-800 mb-6">Job Postings Management (Admin)</h1>
