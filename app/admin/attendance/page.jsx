...
export default function AdminManageAttendancePage() {
  ...
  const fetchStaffUsers = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await fetch("/api/users?roleType=nonStudent"); // Using existing /api/users
      if (!response.ok) {
  ...
  const fetchAttendanceForDate = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch("/api/attendance/bulk", { // Using existing /api/attendance/bulk
        method: "POST",
  ...
  const handleManualAttendanceChange = async (userId, newStatus) => {
    setIsLoading(true);
    try {
      const response = await fetch("/api/attendance/manual", { // Using existing /api/attendance/manual
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          userId,
